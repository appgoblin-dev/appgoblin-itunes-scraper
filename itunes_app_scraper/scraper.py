...
class AppStoreScraper:
    ...
    def get_app_ids_for_query(
        ...
        if "bubbles" not in result or not result["bubbles"]:
            raise AppStoreException(
                f"No results found for search term {term} (country {country}, lang {lang})"
            )
    ...
    def get_app_ids_for_collection(
        ...
        url = (
            "https://itunes.apple.com/WebObjects/MZStoreServices.woa/ws/charts?cc=%s&g=%s&name=%s&limit=%s"
            % params
        )
    ...
    def get_apps_for_developer(self, developer_id, country="nl", lang="", timeout=None):
        ...
        if "results" in result:
            return [
                app for app in result["results"] if app["wrapperType"] == "software"
            ]
        else:
    ...
    def get_app_ids_for_developer(
        ...
        app_ids = [
            app["trackId"]
            for app in apps
            if app.get("wrapperType") == "software" and "trackId" in app
        ]
        return app_ids
    ...
    def get_similar_app_ids_for_app(
        ...
        country = self.get_store_id_for_country(country)
        headers = {"X-Apple-Store-Front": "%s,32" % country, "Accept-Language": lang}
    ...
    def get_app_details(
        ...
        if force:
            ...
            url = (
                "https://itunes.apple.com/lookup?%s=%s&country=%s&entity=software&timestamp=%s"
                % (id_field, app_id, country, timestamp)
            )
        else:
            url = "https://itunes.apple.com/lookup?%s=%s&country=%s&entity=software" % (
                id_field,
                app_id,
                country,
            )

        request_timeout = (10, 30) if timeout is None else (timeout, timeout)
        ...
        if add_ratings:
            try:
                ...
                # Return some details
                self._log_error(
                    country, "Unable to collect ratings for %s" % str(app_id)
                )
        ...
        if flatten:
            for field in app:
                if isinstance(app[field], list):
                    ...
                    app[field] = ", ".join(
                        [
                            "%s star: %s" % (str(key), str(value))
                            for key, value in app[field].items()
                        ]
                    )
    ...
    def get_app_ratings(self, app_id, countries=None, sleep=1, timeout=None):
        ...
        for country in countries:
            url = (
                "https://itunes.apple.com/%s/customer-reviews/id%s?displayable-kind=11"
                % (country, app_id)
            )
