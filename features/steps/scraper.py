...
def step_impl(context, search_term):
    context.results = context.scraper.get_app_ids_for_query(
...
def step_impl(context):
    context.results = context.scraper.get_app_ids_for_collection(
...
def step_impl(context):
    context.results = context.scraper.get_app_ids_for_developer(
...
@when('we search for "{num_apps}" apps')
def step_impl(context, num_apps):
    apps = context.scraper.get_app_ids_for_query(
        "mindful", country="gb", lang="en", num=num_apps
    )
    context.results = list(context.scraper.get_multiple_app_details(apps, country="gb"))


@when('we search for another "{num_apps}" apps')
def step_impl(context, num_apps):
    apps = context.app_id + context.scraper.get_app_ids_for_query(
        "mindful", country="gb", lang="en", num=num_apps
    )
    context.results = list(context.scraper.get_multiple_app_details(apps, country="gb"))


@when('we define an incorrect app id "{app_id}"')
def step_impl(context, app_id):
